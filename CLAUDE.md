...
- **Non-serialized private fields (e.g. a cached `GetComponent<T>()` result) go back to `null` after every domain reload**, even on pre-existing scene objects, because `Awake()` does not re-run for objects that already existed before the reload. During iterative RunCommand-driven development (frequent recompiles), any such field can silently become `null` later. Fix: use a lazy property (`x ?? (x = GetComponent<T>())`) instead of caching once in `Awake()`.
- **`gameObject.SetActive(true)` does not undo custom "is dead" logic.** If a script tracks aliveness via its own field/property (not just `activeSelf`), reactivating the GameObject alone leaves that internal flag stale — any logic gated on it (e.g. an early-return in `Update()`) will silently no-op forever. Provide an explicit `Revive()`-style method that resets the internal flag, don't rely on `SetActive` alone.
